...
[![Two-Wheeled Self-Balancing Robot](https://i.vimeocdn.com/video/2167383678-d726ee7314aeb8796b2be888bdf62cbebdc9eb4ff88cbcb151cd76c5abcec672-d_640x360?&r=pad&region=us)](https://vimeo.com/1200325665)


# Robot Overview
This robot consists of a box-shaped body on top of two wheels controlled by motors. Its goal is to remain upright, maintaining a target angle and resisting the forces of gravity as well as external disturbances. The angle (perpendicular to the ground) where it is balanced is our target angle. It is worth noting that while the target angle might be \\( 0 ^\circ \\), the construction of the robot and its center of gravity relative to the wheel axis could result in a slightly different angle. In this robot's case, I experimentally determined the best target angle to be \\( -1.25 ^\circ \\).

The robot reads the relevant values from an IMU in order to estimate its current tilt angle. It then feeds the difference between this estimate and the target angle to a PD controller, which will then move the motors accordingly. This process repeats in an infinite loop until the robot is powered down.

# Construction
## Parts List
The following components were used in the robot's construction:
...
$$
\hat \theta_ \text{accel} = \text{atan2}(a_y,a_z)
$$
...
Sensor fusion can be done using various algorithms of various complexities. This project uses a simple complementary filter to combine sensor data. The complementary filter takes the form of
...
PD stands for proportional-derivative, which are the two ways we will deal with our error. Given an error signal, it generates a corresponding control signal according to
$$
